...
const Users = () => {
  ...
  const fetchAll = async () => {
    try {
      ...
      usersResponse.data.sort((a, b) => { // sorts names on alphabetical order
        if (a.name < b.name) {
          return -1;
  ...
  const UsersTable = () => {
    ...
    const itemsToShow = users.slice(startIndex, startIndex + itemsPerPage);
    ...
    const deleteUser = async (user_id) => {
      try {
        ...
        await sales.delete(`users?user_id=${user_id}`);
        setError(<p className='text-success fw-bold'>Gebruiker is succesvol verwijderd</p>)
        fetchAll();
      } catch (error) {
        console.warn(error);
          ...
          <table className="table table-striped table-hover	table-borderless table-primary align-middle">
            ...
            <tbody>
              {itemsToShow.map((user) => (
                <tr key={user.user_id}>
                  <td>{user.name}</td>
                  <td><p onClick={e => navigate(`/admin/users/${user.user_id}`)} className='btn btn-primary'>Aanpassen</p></td>
                  <td><p onClick={e => deleteUser(user.user_id)} className='btn btn-danger'>Verwijder</p></td>
                </tr>
              ))}
            </tbody>
          </table>
